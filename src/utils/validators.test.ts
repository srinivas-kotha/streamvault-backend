...
describe('downloadQueueSchema', () => {
  it('rejects non-positive vod_id', () => {
    const result = downloadQueueSchema.safeParse({ vod_id: 0, vod_name: 'Test' });
    expect(result.success).toBe(false);
  });

  it('rejects negative vod_id', () => {
    const result = downloadQueueSchema.safeParse({ vod_id: -5, vod_name: 'Test' });
    expect(result.success).toBe(false);
  });

  it('accepts valid input', () => {
    const result = downloadQueueSchema.safeParse({ vod_id: 42, vod_name: 'Movie Name' });
    expect(result.success).toBe(true);
  });
});
